...
/*   Updated: 2025/06/27 20:47:26 by enrgil-p         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	execute_cmd_2(t_pipex_data pipex_data, int *pipe_fd)
{
	...
	if (close(pipe_fd[1]) == -1)
		close_a_fd_and_error(pipe_fd[0], "close pipe_fd 1", pipex_data);
	...
	if (dup2(fd_outfile, 1) == -1 || dup2(pipe_fd[0], 0) == -1)
		close_fds_and_error(fd_outfile, pipe_fd[0], "dup2", pipex_data);
	if (close(fd_outfile) == -1)
		close_a_fd_and_error(pipe_fd[0], "close outfile", pipex_data);
...
void	execute_cmd_1(t_pipex_data pipex_data, int *pipe_fd)
{
	...
	if (close(pipe_fd[0]) == -1)
		close_a_fd_and_error(pipe_fd[1], "close pipe_fd 0", pipex_data);
	...
	if (close(fd_infile) == -1)
		close_a_fd_and_error(pipe_fd[1], "close infile", pipex_data);
	if (close(pipe_fd[1]) == -1)
		error_happened("close pipe_fd[1] in cmd_1", pipex_data);
