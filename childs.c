...
/*   Updated: 2025/06/27 20:57:22 by enrgil-p         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	execute_cmd_2(t_pipex_data pipex_data, int *pipe_fd)
{
	...
	if (close(pipe_fd[0]) == -1)
		error_happened("Fail close pipe_fd[1] in cmd_1.", pipex_data);
...
void	execute_cmd_1(t_pipex_data pipex_data, int *pipe_fd)
{
	...
	if (close(pipe_fd[0]) == -1)
		close_a_fd_and_error(pipe_fd[1], "Fail close.", pipex_data);
